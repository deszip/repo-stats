...
- (STCommit *)getStats:(NSString *)commitHash {
    ...
    [self getCount:&lineCount length:&fileLength];
    
    STCommit *commit = [STCommit new];
    commit.commitDate = [self commitDate:commitHash];
    commit.commitHash = commitHash;
    commit.totalLineCount = lineCount;
    commit.additionalLineCount = fileLength;
...
- (void)getCount:(inout NSUInteger *)count length:(inout NSUInteger *)length {
    ...
    NSUInteger totalLineCount = 0;
    NSUInteger totalTestsLineCount = 0;
...
//    NSUInteger totalLength = 0;
    ...
    for (NSURL *itemURL in enumerator) {
        if ([supportedExtensions containsObject:itemURL.pathExtension]) {
            @autoreleasepool {   
                if ([itemURL.path containsString:@"/Pods/"]) {
                    continue;
                }
                NSError *error;
                ...
                if (contents) {
                    NSUInteger lineCount = [contents componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]].count;
                    if ([contents containsString:@"XCTest"]) {
                        totalTestsLineCount += lineCount;
                    } else {
...
//    *length = totalLineCount / fileCount;
//    *length = fileCount;
    *length = totalTestsLineCount;
}
...
//- (NSUInteger)commitLines:(NSString *)commitHash {
//
//}
